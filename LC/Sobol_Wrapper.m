...
% user controls
Nsample     = 250;                % size of training sample
...
% --- First Homogeneous Term ----------------------------------------------
        ...
        cmd=['cp output.out output_1_',num2str(n),'.out'];
        system(cmd);%wangxi change
% --- Second Homogeneous Term ---------------------------------------------
        run_parms   = Q2(n,:);
        Y2(n,:)       = test_model(run_parms,z,I);
        cmd=['cp output.out output_2_',num2str(n),'.out'];
        system(cmd);%wangxi change
% --- Heterogeneous Terms -------------------------------------------------
        for p_dex = 1:length(Parm_dexes)
            ...
            cmd=['cp output.out output_12_',num2str(n),'_',num2str(p_dex),'.out'];
            system(cmd);%wangxi change
            
            run_parms    = Q1(n,:);
            run_parms(p) = Q2(n,p);
            Y21(n,p_dex,:) = test_model(run_parms,z,I);
            cmd=['cp output.out output_21_',num2str(n),'_',num2str(p_dex),'.out'];
            system(cmd);%wangxi change
...
% --- Save Results to File(s) ---------------------------------------------
    Ffname = strcat('FOSI_',num2str(loopdex),'.mat');
    Tfname = strcat('TSI_',num2str(loopdex),'.mat');
    save(Ffname,'FOSI');
    save(Tfname,'TSI');
end%loopdex
