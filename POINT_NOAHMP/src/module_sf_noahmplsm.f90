...
!------------------------------------------------------------------------------------------!
  ...
  REAL, DIMENSION (1:NSLTYPE) :: BB,DRYSMC,F11,                           &
        MAXSMC, REFSMC,SATPSI,SATDK,SATDW, WLTSMC,QTZ  !!wangxi
!------------------------------------------------------------------------------------------!
...
!    EPS    = -1.E36
!
!    open(15, file="parm_tables/MPTABLE.TBL", status='old', form='formatted', action='read', iostat=ierr)
!    if (ierr /= 0) then
!       write(*,'("****** Error ******************************************************")')
...
! water and energy balance check  !!wangxi why not check???
...
!    print*, 'IRG=',IRG,'IRB=',IRB,'IRC=',IRC,'FVEG=',FVEG,'FIRA=',FIRA
    ...
    IF(FIRE <=0.) THEN
       ...
       WRITE(6,*) 'LWDN=',LWDN,'FIRA=',FIRA,'SNOWH=',SNOWH,'IRG=',IRG,'IRB=',IRB,'IRC=',IRC,'FVEG=',FVEG
       stop "STOP in Noah-MP"
    END IF
...
!reduce fluxes at the bottom boundaries accordingly
  ...
  ENDIF
    ...
    SMC = SH2O + SICE  !!wangxi fix the bug
...
! respiration
     ...
     RSSTEM  = RMS25*(STMASS*1E-3)*TF *RF* 12.e-6         ! g/m2/s
...
! leaf and root turnover per time step
     ...
     LFTOVR = LTOVRC*1.E-6*LFMASS
     STTOVR = LTOVRC*1.E-6*STMASS
